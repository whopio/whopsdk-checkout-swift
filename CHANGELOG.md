...
### Added
- `presentApplePay(_:)` to trigger the Apple Pay sheet from your own UI, in direct response to a user interaction

### Changed
- Purchases are now attributed with granular SDK source values (`swift_apple_pay`, `swift_web`) instead of generic `swift`/`web`

## [0.3.0] - 2026-03-17

### Added
- Apple Pay support for service-based apps via `Checkout.ApplePayButton`

### Changed
- Renamed `PaymentMethod.apple` to `PaymentMethod.storeKit`
